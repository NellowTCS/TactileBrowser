...
char *tactilebrowser_resolve_url(const char *base_url,
  ...
  if (base_url) {
    ...
    url = lxb_url_parse(parser, nullptr, (const lxb_char_t *)base_url,
                        strlen(base_url));
    if (!url) {
      lxb_url_parser_destroy(parser, true);
      return nullptr;
    ...
    url = lxb_url_parse(parser, url, (const lxb_char_t *)candidate_url,
                        strlen(candidate_url));
  } else {
    // Just parse candidate
    url = lxb_url_parse(parser, nullptr, (const lxb_char_t *)candidate_url,
                        strlen(candidate_url));
  }
...
char *tactilebrowser_extract_path(const char *absolute_url) {
  ...
  lxb_url_t *url = lxb_url_parse(
      parser, nullptr, (const lxb_char_t *)absolute_url, strlen(absolute_url));
  if (!url) {
    lxb_url_parser_destroy(parser, true);
    return nullptr;
  }

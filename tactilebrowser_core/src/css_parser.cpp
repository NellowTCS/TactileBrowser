...
static const char* skip_whitespace_and_comments(const char* ptr, const char* end) {
    while (ptr < end) {
...
static const char* skip_string_literal(const char* ptr, const char* end, char quote) {
...
static void css_rules_store_entry(char* selector,
                                  size_t selector_len,
                                  char* declarations,
                                  bool case_insensitive) {
    ...
    CssRuleEntry* entry = css_rules_head;
    while (entry) {
        if (entry->length == selector_len && entry->case_insensitive == case_insensitive) {
            bool match = false;
            if (case_insensitive) {
            ...
            if (match) {
                ...
                size_t needs_semicolon = (existing_len > 0 && entry->declarations[existing_len - 1] != ';') ? 1 : 0;
                char* buffer = (char*)realloc(entry->declarations, existing_len + needs_semicolon + new_len + 1);
                if (!buffer) {
                    free(selector);
                    free(declarations);
                    return;
        ...
        while (sel_ptr < selector_end) {
            ...
            if (selector) {
                if (selector[0] != '@' && selector_is_simple(selector)) {
                    ...
                    if (block_value) {
                        css_rules_store_entry(selector, strlen(selector), block_value, case_insensitive);
                    } else {
                        free(selector);
                    }
                } else {
...
bool css_parser_parse_color_value(const char* value, uint32_t* color_out) {
    ...
    while (rule) {
        if (rule->type == LXB_CSS_RULE_DECLARATION) {
            ...
            } else if (decl->type == LXB_CSS_PROPERTY_BACKGROUND_COLOR && decl->u.background_color) {

...
private:
  ...
  static void *lvgl_create_text_area(Renderer *renderer, const char *value,
                                     int x, int y, int width, int height) {
    return lvgl_create_text_widget(renderer, value, nullptr, x, y, width,
                                   height, true);
  }

  static void *lvgl_create_container(Renderer *renderer, int x, int y,
                                     int width, int height) {
  ...
  static void lvgl_set_bg_gradient(Renderer *renderer, void *widget,
    ...
    lv_obj_set_style_bg_grad_dir(
        obj, gradient_dir_from_angle(gradient->angle_deg), 0);
...
void load_url(const char *url, int tab_index) {
  if (!url || strlen(url) == 0 || tab_index >= MAX_TABS)
    return;
  ...
  if (resolved.empty()) {
    ...
    lv_label_set_text(
        error_label, "Unable to resolve relative path without an active site.");
  ...
  if (!has_http_scheme(final_url)) {
    ...
    lv_label_set_text(error_label,
                      "Invalid URL format. Please use http(s) links.");
    lv_obj_center(error_label);
    lv_obj_set_style_text_color(error_label, lv_color_hex(0xFF6B6B), 0);
    return;
  }
  ...
  RenderResult result =
      tactilebrowser_render_url(final_url, tabs[tab_index].content_area,
                                SCREEN_WIDTH - 40, SCREEN_HEIGHT - 100);
...
static void new_tab_event_cb(lv_event_t *e) {
  if (tab_count < MAX_TABS) {
    ...
    lv_obj_set_scrollbar_mode(tabs[tab_count].content_area,
                              LV_SCROLLBAR_MODE_AUTO);
    lv_obj_set_style_bg_color(tabs[tab_count].content_area,
                              lv_color_hex(0x1E1E1E), 0);
...
static void handle_mouse_event(SDL_Event *event) {
  // Handle mouse events manually since lv_sdl_mouse_handler might not be
  // available
...
void init_browser_ui(void) {
  ...
  lv_obj_add_event_cb(tabview, tab_changed_event_cb, LV_EVENT_VALUE_CHANGED,
                      NULL);

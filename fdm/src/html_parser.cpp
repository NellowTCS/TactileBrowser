...
static lxb_html_document_t *html_parser_parse_html(const char *html,
                                                   size_t length) {
  ...
  lxb_status_t status =
      lxb_html_document_parse(document, (const lxb_char_t *)html, length);
...
static char *html_parser_extract_text_content(lxb_html_document_t *document) {
  ...
  size_t text_len = 0;
  lxb_char_t *text =
      lxb_dom_node_text_content(lxb_dom_interface_node(body), &text_len);
...
html_parser_find_body_element(lxb_html_document_t *document) {
  if (!document)
    return NULL;
  ...
  lxb_dom_collection_t *collection =
      lxb_dom_collection_make(lxb_dom_interface_document(document), 4);
  if (!collection)
    return NULL;

  if (lxb_dom_elements_by_tag_name(root, collection, (const lxb_char_t *)"body",
                                   4) != LXB_STATUS_OK ||
...
static const char *html_parser_get_element_tag(lxb_dom_element_t *element,
                                               size_t *length) {
  return (const char *)lxb_dom_element_local_name(element, length);
}

static char *html_parser_get_element_text(lxb_dom_element_t *element,
                                          size_t *length) {
  if (!element) {
    if (length)
      *length = 0;
  ...
  size_t text_len = 0;
  lxb_char_t *text =
      lxb_dom_node_text_content(lxb_dom_interface_node(element), &text_len);
  if (length)
    *length = text_len;
...
static const char *html_parser_get_element_attr(lxb_dom_element_t *element,
                                                const char *attr_name,
                                                size_t *length) {
  return (const char *)lxb_dom_element_get_attribute(
      element, (const lxb_char_t *)attr_name, strlen(attr_name), length);
}
...
ElementType get_element_type_from_element(lxb_dom_element_t *element) {
  ...
  switch (tag_id) {
  case LXB_TAG_H1:
    return ELEMENT_HEADING1;
  case LXB_TAG_H2:
    return ELEMENT_HEADING2;
  case LXB_TAG_H3:
    return ELEMENT_HEADING3;
  case LXB_TAG_H4:
    return ELEMENT_HEADING4;
  case LXB_TAG_H5:
    return ELEMENT_HEADING5;
  case LXB_TAG_H6:
    return ELEMENT_HEADING6;
  case LXB_TAG_P:
  ...
  case LXB_TAG_B:
    return ELEMENT_BOLD;
  case LXB_TAG_I:
    return ELEMENT_ITALIC;
  case LXB_TAG_U:
    return ELEMENT_UNDERLINE;
  case LXB_TAG_LI:
    return ELEMENT_LIST_ITEM;
  case LXB_TAG_OL:
    return ELEMENT_ORDERED_LIST;
  case LXB_TAG_UL:
    return ELEMENT_UNORDERED_LIST;
  case LXB_TAG_IMG:
    return ELEMENT_IMAGE;
  case LXB_TAG_BR:
    return ELEMENT_BREAK;
  case LXB_TAG_HR:
    return ELEMENT_HORIZONTAL_RULE;
  default:
    return ELEMENT_UNKNOWN;
  }
}